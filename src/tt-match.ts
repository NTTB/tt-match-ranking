...
export class TTMatch<T>{
  ...
  addSet(set: TTSet, homePlayerId: number, awayPlayerId: number): number {
    if (homePlayerId == awayPlayerId) throw new Error("The homePlayerId and awayPlayerId cannot be the same");
    if (!this.isPlayerIdKnown(homePlayerId)) throw new Error("The homePlayerId is not part of this match");
    if (!this.isPlayerIdKnown(awayPlayerId)) throw new Error("The awayPlayerId is not part of this match");
    if (this.isSetKnown(set)) throw new Error("The set was already added to the match");
