...
`("parseMatchRules(\"$input\")", ({ winning, losing, input }) => {
  ...
  test(`should set victoryPoints to ${winning}`, () => expect(rules.victoryPoints).toBe(winning));
  test(`should set defeatPoints to ${losing}`, () => expect(rules.defeatPoints).toBe(losing));
});
...
describe("assertMatchRules(...)", () => {
  test("should throw when lose score is higher equal to win score", () => {
    ...
    expect(act).toThrowError(/The victoryPoints should be higher than the defeatPoints/);
  });
  ...
  `("should not throw when a valid configuration is provided", ({ winning, losing }) => {
    const act = () => assertMatchRules({ victoryPoints: winning, defeatPoints: losing });
